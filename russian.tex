...
%\setlength\bibitemsep{0.85\itemsep}
...
{\contentslabel{1.8em}}
{\hspace*{-1.8em}}
{\titlerule*[1pc]{.\iftocodd\global\tocoddfalse\else\hbox to0.5pc{\hss}\global\tocoddtrue\fi}\contentspage}

\titlecontents{subsection}
[5.6em]
{\vspace{0.7pt}}
{\contentslabel{3.05em}}
{\hspace*{-3.05em}}
{\titlerule*[1pc]{.\iftocodd\global\tocoddfalse\else\hbox to0.5pc{\hss}\global\tocoddtrue\fi}\contentspage}
...
\title{\sffamily\myLaTeX\ в 2025 году}
...
\begin{document}
\vspace*{7cm}
\pdfbookmark[1]{LaTeX в 2025 году}{title}%
\vspace{-\baselineskip}%
...
которые мне представляются разумными в 2025 году.
...
\pagetotal=0cm
\savepagegoal=\pagegoal
\pagegoal=7cm
\newsavebox\tblrsavebox
\savebox{\tblrsavebox}{%
\begin{tcolorbox}[colback=white,colframe=white]%
\DefTblrTemplate{conthead-text}{default}{(продолжение)}%
\DefTblrTemplate{contfoot-text}{default}{Продолжение следует}%
...
\pagegoal=\savepagegoal
\pagetotal=\savepagetotal
\begin{figure}[t]
\usebox{\tblrsavebox}
\caption{Результат вёрстки таблицы с помощью \package{longtblr} (пакет \package{tabularray})}\label{tabularray4}
\end{figure}
Отмечу, что пакет \package{tabularray} все ещё находится в~разработке, поэтому в~нём иногда попадаются ошибки. Кроме того, он пока еще не~описан ни в~одном русскоязычном
учебнике, так что для его освоения придётся прочитать документацию.
...
\section{Предметный указатель, глоссарий: upmendex}\label{sec:Index}
...
Поэтому сейчас я отказался от использования \exe{xindy} и перешёл на недавно появившийся \exe{upmendex} \parencite{ctan-upmendex}. Краткую инструкцию по~настройке \exe{xindy} вы можете найти в приложении~\ref{sec:xindy}, она может пригодиться для случаев, когда \exe{upmendex} оказывается недостаточно гибким: он, например, поддерживает глубину вложения указателей не более пяти уровней (и до недавнего времени поддерживал только три уровня).
...
\section{Введение}
Текст введения\index{введение} с библиографическими
ссылками~\parencite{esin1950}.

\section{Основной результат}
Текст основной части работы\index{результат} тоже
со ссылками~\parencite{eliseeva2010}.
...
\newcommand{\mathsamplerus}[1]{%
  ...
  \begin{equation}
    g(t) = \mathop{\textrm{lim}}_{\varepsilon\to0}\mathop{\textrm{lim}}_{\lambda\to\infty}\varepsilon^{-1}
  ...
  \begin{equation}
    ...
    \text{$z\to\infty$ при $\mathopen|\mathop{\textrm{arg}} z\mathclose|<\pi$.}
  \end{equation}
  ...
  \begin{equation*}
    \frac{1}{2\pi i}\int_\gamma f=\sum_{k=1}^m n(\gamma;a_k)\mathop{\textrm{Res}}(f;a_k).
  \end{equation*}
  ...
  \begin{equation*}
    \mathop{\textrm{max}}\{|f(z)|\colon z\in\overline{G}\}=\mathop{\textrm{max}}\{|f(z)|\colon z\in\partial G\}.
  \end{equation*}

  \example{Тождество \emph{Якоби}}
  Определим \emph{тета-функцию $\theta$} равенством
  \begin{equation*}
    \theta(t) = \sum_{n=-\infty}^\infty\mathop{\textrm{exp}}(-\pi n^2t),\quad t>0.
  \end{equation*}
  Тогда
  \begin{equation*}
...
\clearpage
\subsection[Cyrillic Modern \& Latin Modern Math]{Cyrillic Modern \parencite{ctan-cyrillic-modern} \& Latin Modern Math \parencite{ctan-lm-math}}
...
\setcounter{equation}{0}
\setmainfont{Erewhon}[ItalicFont={Erewhon Italic},BoldFont={Erewhon Bold}]
...
\clearpage
\normalsize
\subsection[IBM Plex Serif \& IBM Plex Math]{IBM Plex Serif \parencite{ctan-plex-otf} \& IBM Plex Math \parencite{font-plex-math}}
\setcounter{equation}{0}
\setmainfont[Scale=0.97]{IBM Plex Serif}
\setmathfont[version=ibmplex,mathrm=sym,Scale=0.97]{IBM Plex Math}
\mathversion{ibmplex}
\small
\begin{latexcode}
\setmainfont{IBM Plex Serif}
\setmathfont{IBM Plex Math Math}
\end{latexcode}
\mathsamplerus{ibmplex}
...
\clearpage
\normalsize
\subsection[Lato \& Lete Sans Math]{Lato \parencite{ctan-lato} \& Lete Sans Math \parencite{ctan-lete-sans-math}}
\setcounter{equation}{0}
\setmainfont{Lato}
\setmathfont[version=letesans,mathrm=sym]{Lete Sans Math}
\mathversion{letesans}
\small
\begin{latexcode}
\setmainfont{Lato}
\setmathfont{Lete Sans Math}
\end{latexcode}
\mathsamplerus{letesans}

\clearpage
\normalsize
\subsection[Segoe UI \& Lete Sans Math]{Segoe UI \parencite{ms-typography-segoe-ui} \& Lete Sans Math \parencite{ctan-lete-sans-math}}
\setcounter{equation}{0}
\setmainfont{Segoe UI}
\setmathfont[version=segoeui,mathrm=sym]{Lete Sans Math}
\mathversion{segoeui}
\small
\begin{latexcode}
\setmainfont{Segoe UI}
...
\clearpage
\normalsize
\subsection[Noto Sans \& Noto Sans Math]{Noto Sans \parencite{ctan-noto} \& Noto Sans Math \parencite{font-noto-math}}
\setcounter{equation}{0}
\setmainfont{Noto Sans}
\setmathfont[version=notosans,mathrm=sym]{NotoSansMath-Regular.otf}
\mathversion{notosans}
\small
\begin{latexcode}
\setmainfont{Noto Sans}
\setmathfont{Noto Sans Math}
\end{latexcode}
\mathsamplerus{notosans}

\clearpage
\normalsize
\subsection[Arsenal \& Arsenal Math]{Arsenal \parencite{ctan-arsenal} \& Arsenal Math \parencite{ctan-arsenal-math}}
\setcounter{equation}{0}
\setmainfont{Arsenal}
\setmathfont[version=arsenal,mathrm=sym]{Arsenal Math}
\mathversion{arsenal}
\small
\begin{latexcode}
\setmainfont{Arsenal}
\setmathfont{Arsenal Math}
\end{latexcode}
\mathsamplerus{arsenal}
...
\clearpage
\normalsize
\setlength\bibitemsep{0.27\baselineskip}%
\phantomsection
...
\phantomsection
\addcontentsline{toc}{section}{Ссылки на вопросы, обсуждения и форумы}%
\printbibliography[keyword=question,title={Ссылки на вопросы, обсуждения и форумы}]
\pagebreak
\phantomsection
